...
def collate_fn_humming_fixed(samples):
    start_seconds = [x for _, x, _ in samples]
    total_seconds = [x for _, _, x in samples]
    samples = [x for x, _, _ in samples]
    ...
    for sample in samples:
        assert "humming" in sample, "样本中必须有 humming.wav"
        in_track = sample["humming"]

        out_keys = [k for k in sample.keys() if k != "humming"]
        out_track = torch.stack([sample[k] for k in out_keys]).sum(dim=0, keepdim=True)

        outputs.append(out_track)
        inputs.append(in_track)
        prompts.append(f"in: humming; out: {', '.join(out_keys)}")

    return torch.stack(outputs), torch.stack(inputs), prompts, start_seconds, total_seconds

def collate_fn_humming_multitrack(samples, required_stems=None):
    """
    每个 sample 是一个 dict: {"humming": tensor, "vocals": tensor, ...}
    返回：
    - inputs: Tensor [B, 1, T]
    - targets: Dict[str, Tensor[B, 1, T]]，按 stem 分开
    - prompts: List[str]
    """
    start_seconds = [x for _, x, _ in samples]
    total_seconds = [x for _, _, x in samples]
    ...
    for sample in samples:
        assert "humming" in sample
        inputs.append(sample["humming"])  # [1, T]

        for stem in sample:
            if stem == "humming":
                continue
            if required_stems is not None and stem not in required_stems:
                continue
            if stem not in targets:
                targets[stem] = []
            targets[stem].append(sample[stem])  # [1, T]

    # stack batch
    inputs = torch.stack(inputs)  # [B, 1, T]
    targets = {k: torch.stack(v) for k, v in targets.items()}  # [B, 1, T] per stem
    prompts = [f"in: humming; out: {', '.join(targets.keys())}"] * len(samples)

    return inputs, targets, prompts, start_seconds, total_seconds
